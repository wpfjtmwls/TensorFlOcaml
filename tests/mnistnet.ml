...
module MnistNet : SUBGRAPH = struct
    ...
    let create in_list name graph graphst =
        ...
        let (b1, graph) = graph |> Graph.variable ~prefix:name [1;300] in
        ...
        let (b2, graph) = graph |> Graph.variable ~prefix:name [1;100] in
        ...
        let (b3, graph) = graph |> Graph.variable ~prefix:name [1;10] in
        ...
        let logger = Some {filename="mnist-loss.log"; interval=32; counter=ref 0} in
        ...
        let graphstate = GraphState.(graphst    
                        ...
                        |> add_node b1 (Arr.uniform [|1;300|])
                        |> add_node b2 (Arr.uniform [|1;100|])
                        |> add_node b3 (Arr.uniform [|1;10|])
        ) in
