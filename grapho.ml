  ...
  let rec backprop_graddesc node grad lr st =
    ...
    | Operation op -> begin
      ...
      | MatMul (a, b) ->
        ...
        let _ = Printf.printf "grad: %s \n" (grad |> Arr.shape |> string_of_shape); in
        let st1 = backprop_graddesc a (Arr.mul grad (b_val |> Arr.transpose)) lr st in
        let st2 = backprop_graddesc b (Arr.mul (a_val |> Arr.transpose) grad) lr st in
